...
def group(values: tp.List[T], n: int) -> tp.List[tp.List[T]]:
    ...
    [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    """
    return [[values[i] for i in range(j, n + j)] for j in range(0, n ** 2, n)]
...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    ['2', '8', '.', '.', '.', '5', '.', '7', '9']
    """
    r0, c0 = pos[0] - pos[0] % 3, pos[1] - pos[1] % 3
    return [grid[i][j] for i in range(r0, r0 + 3) for j in range(c0, c0 + 3)]
...
def generate_sudoku(N: int) -> tp.List[tp.List[str]]:
    ...
    True
    """
    initial_grid = [["." for i in range(9)] for j in range(9)]
    while True:
